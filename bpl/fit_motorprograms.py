...
from bpl.PS import PS


def fit_motorprograms(I, K, verbose, include_mcmc, fast_mode)
...
for i=1:nl
init_scores(i) = scoreMP_NoRel(initMP
{i}, lib);
end
...
finalMP
{i} = SearchForParse(initMP
{i}, lib, verbose, fast_mode);
end
final_scores = zeros(nl, 1);
for i=1:nl
final_scores(i) = scoreMP(finalMP
{i}, lib);
end
...
if include_mcmc
    ...
    for j=1:nl
    fprintf(1, '%d,', j);
    samplesM = RunMCMCType(finalMP
    {j}, lib);
    nsamp = numel(samplesM);
    for i=1:nsamp
    samplesM
    {i}.lightweight; % reduce
    memory
    size
end
samples_type
